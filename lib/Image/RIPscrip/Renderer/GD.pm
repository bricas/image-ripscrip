...
has 'state' =>
    ( is => 'rw', isa => 'HashRef', lazy => 1, builder => '_build_state' );
...
sub _command_T {    # text
    ...
    $state->{ canvas }->string(
        GD::Font->Small, @{ $state->{ location } },
        @args,           $state->{ draw_color }
    );
}
...
sub _command_at {    # text at location
    ...
    # TODO: need to move location after text
    $state->{ canvas }
        ->string( GD::Font->Small, @args, $state->{ draw_color } );
}
...
sub _command_C {     # circle
    ...
    my $size  = $args[ 2 ] * 2;
    $state->{ canvas }
        ->ellipse( @args[ 0, 1 ], $size, $size, $state->{ draw_color } );
}
...
sub _command_o {     # filled ellipse
    ...
    my $h     = $args[ 3 ] * 2;
    $state->{ canvas }
        ->filledEllipse( @args[ 0, 1 ], $w, $h, $state->{ fill_color } );
    $state->{ canvas }
        ->ellipse( @args[ 0, 1 ], $w, $h, $state->{ draw_color } );
}
...
sub _command_1P {    # paste from clipboard
    ...
    my $clip  = $state->{ clip };
    $state->{ canvas }
        ->copy( $clip, @args[ 0 .. 1 ], 0, 0, $clip->getBounds );
}
